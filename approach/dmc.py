...
class Appr(Inc_Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--aux-dataset', default='mnist', type=str, required=False,
    ...
    def pre_train_process(self, t, trn_loader):
        ...
        if t > 0:
            ...
            # pre_cls = self.active_classes[:-self.classes_per_task]
            with torch.no_grad():
                pre_cls = list(range(self.model.task_cls[:t].cumsum(0)[-1]))
                self.model_new.heads.weight[pre_cls,:] = self.model_new.heads.weight[pre_cls,:].zero_()
                self.model_new.heads.bias[pre_cls] = self.model_new.heads.bias[pre_cls].zero_()
                self.model_new.heads.weight[-self.model.task_cls[t+1:].sum():,:] = self.model_new.heads.weight[-self.model.task_cls[t+1:].sum():,:].zero_()
                self.model_new.heads.bias[-self.model.task_cls[t+1:].sum():] = self.model_new.heads.bias[-self.model.task_cls[t+1:].sum():].zero_()
        else:
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        if t > 0:
            ...
            new_trainer.active_classes = self.active_classes
            new_trainer.classes_per_task = self.classes_per_task
            ...
            student_trainer.active_classes = self.active_classes
            student_trainer.classes_per_task = self.classes_per_task
...
class StudentTrainer(Inc_Learning_Appr):
    ...
    def criterion(self, t, outputs, targets_old, targets_new=None):
        ...
        with torch.no_grad():
            ...
            pre_cls = list(range(self.model.task_cls[:t].cumsum(0)[-1]))
            targets = torch.cat((targets_old[:, pre_cls],targets_new[:, self.active_classes[-self.classes_per_task:]]), dim=1)
        ...
        return torch.nn.functional.mse_loss(outputs, targets.detach(), reduction='mean')
