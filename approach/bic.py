...
class Appr(Inc_Learning_Appr):
    ...
    def bias_forward(self, t, outputs):
        """Utility function --- inspired by https://github.com/sairin1202/BIC"""
        bic_outputs = []
        for m in range(t+1):
            bic_outputs.append(self.bias_layers[m](outputs))
        return bic_outputs
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        if t>0:
            num_old_cls = self.model.task_cls[-2]
        else:
            num_old_cls = 0
        if self.exemplars_dataset.max_num_exemplars != 0:
        ...
        if hasattr(trn_loader.dataset,"index"):
            # num_cls = np.unique(trn_loader.dataset.targets)
            labels = np.array(trn_loader.dataset.targets)
        elif hasattr(trn_loader.dataset,"dataset"):
            # num_cls = np.unique(np.array(trn_loader.dataset.dataset.targets)[trn_loader.dataset.sub_indeces])
            labels = np.array(trn_loader.dataset.dataset.targets)[trn_loader.dataset.sub_indeces]
        elif hasattr(trn_loader.dataset,"images"):
            # num_cls = np.unique(np.array(trn_loader.dataset.labels))
            labels = np.array(trn_loader.dataset.labels)
        elif hasattr(trn_loader.dataset,"tensors"):
            # num_cls = torch.unique(trn_loader.dataset.tensors[1])
            labels =trn_loader.dataset.tensors[1]
        ...
        for curr_cls in range(num_old_cls, num_cls):
            ...
            cls_ind = np.where(np.asarray(labels) == curr_cls)[0]
            ...
            if hasattr(trn_loader.dataset,"index"):
                self.x_valid_exemplars[curr_cls] = [trn_loader.dataset.samples[idx] for idx in cls_ind[:num_val_ex_cls]]
                self.y_valid_exemplars[curr_cls] = [trn_loader.dataset.targets[idx] for idx in cls_ind[:num_val_ex_cls]]
            elif hasattr(trn_loader.dataset,"dataset"):
                val_dataset = deepcopy(trn_loader.dataset)
                val_dataset.sub_indeces = cls_ind[:num_val_ex_cls].tolist()
                x_buf = []
                y_buf = []
                for x,y in val_dataset:
                    x_buf.append(x)
                    y_buf.append(y)
                self.x_valid_exemplars[curr_cls] = x_buf
                self.y_valid_exemplars[curr_cls] = y_buf
                del x_buf,y_buf,val_dataset
            elif hasattr(trn_loader.dataset,"images"):
                ...
                self.x_valid_exemplars[curr_cls] = [trn_loader.dataset.tensors[0][idx] for idx in cls_ind[:num_val_ex_cls]]
                self.y_valid_exemplars[curr_cls] = [trn_loader.dataset.tensors[1][idx] for idx in cls_ind[:num_val_ex_cls]]
            non_selected.extend(cls_ind[num_val_ex_cls:])

        # remove selected samples from the validation data used during training
        if hasattr(trn_loader.dataset,"index"):
            trn_loader.dataset.index = np.array(trn_loader.dataset.index)[non_selected].tolist()
            trn_loader.dataset.samples = trn_loader.dataset.samples[non_selected]
            trn_loader.dataset.targets = trn_loader.dataset.targets[non_selected]
        elif hasattr(trn_loader.dataset,"dataset"):
            trn_loader.dataset.sub_indeces = np.array(trn_loader.dataset.sub_indeces)[non_selected].tolist()
        elif hasattr(trn_loader.dataset,"images"):
            trn_loader.dataset.images = trn_loader.dataset.images[non_selected]
            trn_loader.dataset.labels = trn_loader.dataset.labels[non_selected]
        elif hasattr(trn_loader.dataset,"tensors"):
            trn_loader.dataset.tensors = (trn_loader.dataset.tensors[0][non_selected],trn_loader.dataset.tensors[1][non_selected])
        ...
        if t > 0:
            ...
            bic_val_loader = DataLoader(bic_val_dataset, batch_size=self.batch_size, shuffle=True,
            ...
            for e in range(self.bias_epochs):
                ...
                for inputs, targets in bic_val_loader:
                    ...
                    with torch.no_grad():
                        ...
                        old_cls_outs = self.bias_forward(t, outputs[:t])
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            if t > 0:
                ...
                targets_old = self.bias_forward(t, targets_old)  # apply bias correction
            # Forward current model
            outputs = self.model(images.to(self.device))
            outputs = self.bias_forward(t, outputs)  # apply bias correction
