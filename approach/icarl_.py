...
class Appr(Inc_Learning_Appr):
    ...
    def classify(self, task, features, targets):
        ...
        # pred = dists[:, offset:offset + num_cls].argmin(1) # *changed* 
        pred = dists.argmin(1) # *changed* 
        hits_taw = (pred + offset == targets.to(self.device)).float()
        # # change from task-aware multi-head to single-head
        # num_cls = len(self.active_classes)
        # offset = self.active_classes[0]
        # pred = dists[:, self.active_classes].argmin(1)
    ...
    def compute_mean_of_exemplars(self, trn_loader, transform):
        ...
        icarl_loader = DataLoader(copy.deepcopy(trn_loader.dataset), batch_size=trn_loader.batch_size, shuffle=False,
                                    num_workers=trn_loader.num_workers, pin_memory=trn_loader.pin_memory)
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        transforms = val_loader.dataset.dataset.transform if hasattr(val_loader.dataset, 'dataset') else val_loader.dataset.transform
        self.compute_mean_of_exemplars(trn_loader, transforms)
    ...
    def criterion(self, t, outputs, targets, outputs_old=None):
        ...
        if t > 0:
            ...
            g = torch.sigmoid(outputs[:,pre_cls])
            q_i = torch.sigmoid(outputs_old[:,pre_cls])
