...
class Appr(Inc_Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--gradcam-layer', default='conv2', type=str,
    ...
    def train_epoch(self, t, trn_loader):
        ...
        with GradCAM(self.model_old, self.gradcam_layer) as gradcam_old:
            gradcam_old.task_cls = self.model.task_cls
            for images, targets in trn_loader:
                images = images.to(self.device)
                # Forward old model
                attmap_old, outputs_old = gradcam_old(images, return_outputs=True, t=t)
                with GradCAM(self.model, self.gradcam_layer) as gradcam:
                    gradcam.task_cls = self.model.task_cls
                    attmap = gradcam(images, t=t)  # this use eval() pass
    ...
    def eval(self, t, val_loader):
        ...
        with GradCAM(self.model, self.gradcam_layer) as gradcam, \
            ...
            for images, targets in val_loader:
                images = images.to(self.device)
                # Forward old model
                attmap_old, outputs_old = gradcam_old(t, images, return_outputs=True)
                # Forward current model
                attmap, outputs = gradcam(t, images, return_outputs=True)
                loss = self.criterion(t, outputs, targets.to(self.device), outputs_old, attmap, attmap_old)
    ...
    def criterion(self, t, outputs, targets, outputs_old=None, attmap=None, attmap_old=None):
        ...
        if t > 0 and outputs_old is not None:
            ...
            outputs_ = outputs[:,pre_cls]
            outputs_old_ = outputs_old[:,pre_cls]
            loss += self.beta * self.cross_entropy(outputs_,
                                                   outputs_old_, exp=1.0 / 2.0)
...
class GradCAM:
    ...
    def __call__(self, input, class_indices=None, return_outputs=False, t=None):
        ...
        if t is not None:
            cur_cls = list(range(self.task_cls[:t+1].cumsum(0)[-1]))
            model_output = model_output[:,cur_cls]
        logits = model_output
        ...
        # model_output = [o.detach().cpu() for o in model_output]
