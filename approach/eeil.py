...
class Appr(Inc_Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--nepochs-finetuning', default=30, type=int, required=False,
    ...
    def _train_unbalanced(self, t, trn_loader, val_loader):
        ...
        self._train_epoch = 0
        loader = self._get_train_loader(copy.deepcopy(trn_loader), False)
    ...
    def _train_balanced(self, t, trn_loader, val_loader):
        ...
        loader = self._get_train_loader(copy.deepcopy(trn_loader), True)
        super().train_loop(t, loader, val_loader)
        self.lr = orig_lr
    ...
    def _get_train_loader(self, trn_loader, balanced=False):
        ...
        if hasattr(self.exemplars_dataset.dataset,"sub_indeces"):
            sub_indeces = np.random.choice(trn_loader.dataset.sub_indeces, 
                            len(exemplars_ds), replace=False).tolist() if balanced else trn_loader.dataset.sub_indeces
            trn_loader.dataset.cur_task_indeces = trn_loader.dataset.sub_indeces
            trn_loader.dataset.sub_indeces = sub_indeces + self.exemplars_dataset.dataset.sub_indeces
        elif hasattr(self.exemplars_dataset.dataset,"index"):
            if balanced:
                indices = torch.randperm(len(trn_loader.dataset)).tolist()
                index = np.array(indices)[indices[:len(exemplars_ds)]].tolist()
                trn_loader.dataset.cur_task_indeces = trn_loader.dataset.index
                trn_loader.dataset.index = np.array(trn_loader.dataset.index)[index].tolist() + self.exemplars_dataset.dataset.index
                trn_loader.dataset.samples = np.concatenate((trn_loader.dataset.samples[index],self.exemplars_dataset.dataset.samples),0)
                trn_loader.dataset.targets = np.concatenate((trn_loader.dataset.targets[index], self.exemplars_dataset.dataset.targets),0)
            else:
                trn_loader.dataset.cur_task_indeces = trn_loader.dataset.index
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        if t == 0:  # First task is simple training
            ...
            loader = self._train_unbalanced(t, copy.deepcopy(trn_loader), val_loader)
            # Balanced fine-tunning (new + old)
            self._train_balanced(t, copy.deepcopy(trn_loader), val_loader)
    ...
    def criterion(self, t, outputs, targets, outputs_old=None):
        ...
        loss = torch.nn.functional.cross_entropy(outputs[:,list(range(self.model.task_cls[:t+1].cumsum(0)[-1]))], targets)
        # Distilation loss
        if t > 0 and (outputs_old is not None):
            ...
            if self._finetuning_balanced:
                loss += self.lamb * F.binary_cross_entropy(F.softmax(
                    outputs[:, list(range(self.model.task_cls[:t+1].cumsum(0)[-1]))] / self.T, dim=1),
                    F.softmax(outputs_old[:, list(range(self.model.task_cls[:t+1].cumsum(0)[-1]))] / self.T, dim=1))
            else:
                loss += self.lamb * F.binary_cross_entropy(F.softmax(
                    outputs[:, list(range(self.model.task_cls[:t].cumsum(0)[-1]))] / self.T, dim=1),
                    F.softmax(outputs_old[:, list(range(self.model.task_cls[:t].cumsum(0)[-1]))] / self.T, dim=1))
        return loss
