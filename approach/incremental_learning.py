...
class Inc_Learning_Appr:
    ...
    def train(self, t, trn_dataset, val_dataset):
        """Main train structure"""
        trn_loader = utils.get_data_loader(trn_dataset, batch_size=self.batch_size, cuda=self.device,drop_last=True)
        val_loader = utils.get_data_loader(val_dataset, batch_size=self.batch_size, cuda=self.device)
        self.pre_train_process(t, trn_loader)
        self.train_loop(t, trn_loader, val_loader)
        self.post_train_process(t, trn_dataset)
        ...
        if self.warmup_epochs and t > 0:
            ...
            for e in range(self.warmup_epochs):
                ...
                with torch.no_grad():
                    ...
                    for images, targets in trn_loader:
                        outputs = self.model(images.to(self.device))
                        if outputs.shape[0] == self.classes_per_task:
                        ...
                        for m in range(len(pred)):
                            this_task = (self.model.task_cls.cumsum(0) <= targets[m]).sum()
                            pred[m] = outputs[m].argmax() + self.model.task_offset[this_task]
    ...
    def calculate_metrics(self, outputs, targets, t):
        ...
        for m in range(len(pred)):
            this_task = (self.model.task_cls.cumsum(0) <= targets[m]).sum()
            cls_idx = list(range(sum(self.model.task_cls[:this_task]),sum(self.model.task_cls[:this_task+1])))
            outputs_ = outputs[:, cls_idx]
            # pred[m] = outputs[this_task][m].argmax() + self.model.task_offset[this_task]
            pred[m] = outputs_[m].argmax() + sum(self.model.task_cls[:this_task])
        hits_taw = (pred == targets.to(self.device)).float()
        ...
        # Task-Agonostic
        pred = outputs[:, list(range(self.active_classes[-1]+1))].argmax(1)
