...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, nepochs=100, lr=0.05, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
                 ...
                 logger=None, exemplars_dataset=None, lamb=1, alpha_=0.5, damping=0.1, fim_sampling_type='max_pred',
                 fim_num_samples=-1):
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--alpha_', default=0.5, type=float, required=False,
                            help='RWalk alpha_ (default=%(default)s)')
    ...
    def compute_fisher_matrix_diag(self, t, trn_loader):
        ...
        for images, targets in itertools.islice(trn_loader, n_samples_batches):
            ...
            if self.sampling_type == 'true':
                ...
                preds = outputs[:,list(range(self.model.task_cls[:t+1].cumsum(0)[-1]))].argmax(1).flatten()
            elif self.sampling_type == 'multinomial':
                # Use a multinomial sampling to compute the gradients
                probs = torch.nn.functional.softmax(outputs[:,list(range(self.model.task_cls[:t+1].cumsum(0)[-1]))], dim=1)
                preds = torch.multinomial(probs, len(targets)).flatten()

            loss = torch.nn.functional.cross_entropy(outputs[:,list(range(self.model.task_cls[:t+1].cumsum(0)[-1]))], preds)
    ...
    def post_train_process(self, t, trn_loader):
        ...
        curr_fisher = self.compute_fisher_matrix_diag(t, trn_loader)

        # Eq. 10: efficiently update Fisher Information Matrix
        for n in self.fisher.keys():
            # Added option to accumulate fisher over time with a pre-fixed growing alpha_
            if self.alpha_ == -1:
                alpha_ = (sum(self.model.task_cls[:t]) / sum(self.model.task_cls)).to(self.device)
                self.fisher[n] = alpha_ * self.fisher[n] + (1 - alpha_) * curr_fisher[n]
            else:
                self.fisher[n] = self.alpha_ * self.fisher[n] + (1 - self.alpha_) * curr_fisher[n]
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            loss = torch.nn.functional.cross_entropy(outputs[:,list(range(self.model.task_cls[:t+1].cumsum(0)[-1]))], targets.to(self.device))
            self.optimizer.zero_grad()
